...
static uint16_t                 AudioOut_BitResolution = 16U;
static uint32_t                 AudioOut_DataBytes = 2U;
...
{
  return BSP_AUDIO_OUT_InitEx(OutputDevice, Volume, AudioFreq, 16U);
}

uint8_t BSP_AUDIO_OUT_InitEx(uint16_t OutputDevice, uint8_t Volume,
                             uint32_t AudioFreq, uint16_t BitResolution)
{ 
  uint8_t ret = AUDIO_ERROR;
  uint32_t deviceid = 0x00;

  if((BitResolution != 16U) && (BitResolution != 24U))
  {
    return AUDIO_ERROR;
  }
  AudioOut_BitResolution = BitResolution;
  AudioOut_DataBytes = (BitResolution == 24U) ? 4U : 2U;
  ...
  if(ret == AUDIO_OK)
  {
    ...
    /* AIF1 Control (1): I2S format with a 16-bit or 24-bit word length. */
    AUDIO_IO_Write(AUDIO_I2C_ADDRESS, 0x300,
                   (BitResolution == 24U) ? 0x4050U : 0x4010U);
  }
...
uint8_t BSP_AUDIO_OUT_Play(uint16_t* pBuffer, uint32_t Size)
{
  ...
  else
  {
    ...
    if(HAL_SAI_Transmit_DMA(&haudio_out_sai, (uint8_t*) pBuffer,
                            DMA_MAX(Size / AudioOut_DataBytes)) != HAL_OK)
    {
      return AUDIO_ERROR;
    }
    return AUDIO_OK;
...
void HAL_SAI_ErrorCallback(SAI_HandleTypeDef *hsai)
{
  ...
  else if (hsai->Instance == AUDIO_IN_SAIx)
  {
...
{ 
  ...
  if(hsai->Instance == AUDIO_OUT_SAIx)
  {
    ...
    hdma_sai_tx.Init.PeriphDataAlignment =
        (AudioOut_BitResolution == 24U) ? DMA_PDATAALIGN_WORD :
                                          DMA_PDATAALIGN_HALFWORD;
    hdma_sai_tx.Init.MemDataAlignment =
        (AudioOut_BitResolution == 24U) ? DMA_MDATAALIGN_WORD :
...
{ 
  ...
  {
    /* WM8994-only 44.1 kHz family: 11.289474 MHz gives about -11.2 ppm at
       44.1 kHz. S/PDIF is mutually exclusive and programs its own PLLI2S. */
...
static void SAIx_Out_Init(uint32_t AudioFreq)
{
  ...
  haudio_out_sai.Init.DataSize = (AudioOut_BitResolution == 24U) ?
                                  SAI_DATASIZE_24 : SAI_DATASIZE_16;
...
{ 
  ...
  {
    /* SDMMC2 and the four-microphone DFSDM input share DMA2 streams 0/5.
       Finish any short SD DMA transaction before the audio MSP owns them. */
    SD_DMA_BlockForDigitalAudio();
