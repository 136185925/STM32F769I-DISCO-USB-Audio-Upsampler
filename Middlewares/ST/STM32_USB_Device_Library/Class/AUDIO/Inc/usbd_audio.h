...
#define AUDIO_FEEDBACK_FS_PACKET_SIZE                 3U
#define AUDIO_FEEDBACK_HS_PACKET_SIZE                 4U
#define AUDIO_FEEDBACK_MAX_PACKET_SIZE                4U
/* Explicit feedback is refreshed every 2 ms. At full speed the exponent is
 * expressed in 1 ms frames (2^1 ms); at high speed host scheduling uses
 * 125 us microframes, so 2^4 microframes are required for the same period. */
#define AUDIO_FEEDBACK_FS_REFRESH                     1U
#define AUDIO_FEEDBACK_HS_REFRESH                     4U

#define USB_AUDIO_CONFIG_DESC_SIZ                     0x7AU
...
#define AUDIO_CONTROL_SAMPLING_FREQ                   0x01U
...
/* UAC2 request codes no longer encode GET/SET in bRequest. CUR and RANGE
 * use 0x01/0x02 respectively; bit 7 of bmRequestType carries the direction. */
#define AUDIO2_REQ_CUR                                 0x01U
#define AUDIO2_REQ_RANGE                               0x02U
#define AUDIO2_REQ_DIRECTION_IN                        0x80U
...
/* An asynchronous sink must accept the nominal frame count plus one complete
   stereo slot. At 48 kHz this is 49 frames, or 196 bytes per millisecond. */
#define AUDIO_OUT_PACKET                              \
  (uint16_t)((((USBD_AUDIO_FREQ + 999U) / 1000U) + 1U) * 2U * 2U)
...
typedef struct
{
  ...
  uint8_t feedback[AUDIO_FEEDBACK_MAX_PACKET_SIZE];
  uint16_t feedback_sof_countdown;
  uint16_t feedback_target_frame;
  uint8_t feedback_target_valid;
...
typedef struct
{
  ...
  int8_t (*FrequencyCtl)(uint32_t frequency);
  int8_t (*PeriodicTC)(uint8_t *pbuf, uint32_t size, uint8_t cmd);
  int8_t (*GetState)(void);
  uint32_t (*GetFeedback)(void);
} USBD_AUDIO_ItfTypeDef;

typedef enum
{
  USBD_AUDIO_PROTOCOL_UAC1 = 0,
  USBD_AUDIO_PROTOCOL_UAC2
} USBD_AUDIO_ProtocolTypeDef;
...
extern volatile uint32_t usbd_audio_feedback_tx_count;
extern volatile uint32_t usbd_audio_feedback_complete_count;
extern volatile uint32_t usbd_audio_feedback_error_count;
extern volatile uint32_t usbd_audio_feedback_last_q16;
