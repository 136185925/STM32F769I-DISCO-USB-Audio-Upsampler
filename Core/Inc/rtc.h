/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file    rtc.h
  * @brief   This file contains all the function prototypes for
  *          the rtc.c file
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2026 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */
/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __RTC_H__
#define __RTC_H__

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "main.h"

/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

extern RTC_HandleTypeDef hrtc;

/* USER CODE BEGIN Private defines */

#define RTC_LSE_TICK_HZ 1024U

/* USER CODE END Private defines */

void MX_RTC_Init(void);

/* USER CODE BEGIN Prototypes */

void Stopwatch_Toggle(void);
void Stopwatch_UserButtonIRQ(void);
void Stopwatch_Reset(void);
uint8_t Stopwatch_Lap(void);
uint32_t Stopwatch_GetTicks(void);
uint8_t Stopwatch_IsRunning(void);
uint8_t Stopwatch_GetLapCount(void);
uint8_t Stopwatch_GetLapNumber(uint8_t index);
uint32_t Stopwatch_GetLapTicks(uint8_t index);
uint32_t Stopwatch_GetLapTotalTicks(uint8_t index);
uint32_t RTC_LSE_GetTicks(void);

extern volatile uint32_t stopwatch_lse_irq_count;
extern volatile uint32_t stopwatch_control_generation;
extern volatile uint32_t stopwatch_user_button_press_count;
extern volatile uint32_t stopwatch_user_button_debounce_count;

/* USER CODE END Prototypes */

#ifdef __cplusplus
}
#endif

#endif /* __RTC_H__ */

